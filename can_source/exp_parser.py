...
class ExpParser:
    ...
    def parse_exp10(cls):
        ...
        while True:
            ...
            if now.value in ('>', '>=', '<', '<=', '==', '!=', kw_is):
                op = now.value
                F.skip_once()
                exp = can_ast.BinopExp(op if op != kw_is else '==', exp, cls.parse_exp9())
    ...
    def parse_exp9(cls):
        exp = cls.parse_exp8()
        while F.try_look_ahead().typ == TokenType.OP_BOR or \
            F.try_look_ahead().value == "或":
            F.skip_once()
    ...
    def parse_exp8(cls):
        exp = cls.parse_exp7()
        while F.try_look_ahead().typ == TokenType.OP_WAVE or \
            F.try_look_ahead().value == "異或":
            F.skip_once()
            exp = can_ast.BinopExp('^', exp, cls.parse_exp8())
        return exp
    ...
    def parse_exp7(cls):
        exp = cls.parse_exp6()
        while F.try_look_ahead().typ == TokenType.OP_BAND or \
            F.try_look_ahead().value == '與':
            F.skip_once()
            exp = can_ast.BinopExp('&', exp, cls.parse_exp8())
        return exp
    ...
    def parse_exp3(cls):
        exp = cls.parse_exp2()
        while True:
            if F.try_look_ahead().typ in (TokenType.OP_MUL, TokenType.OP_MOD, 
    ...
    @classmethod
    def parse_exp2(cls):
        if  F.try_look_ahead().value == 'not' or \
            F.try_look_ahead().value == '-' or \
            F.try_look_ahead().value == '~':

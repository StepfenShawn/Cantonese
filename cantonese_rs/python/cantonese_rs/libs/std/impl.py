"""Core Cantonese runtime: built-in types and functions."""
...
class List(list):
    """Cantonese list type with Cantonese-named methods."""
...
class Str(str):
    """Cantonese string type with Cantonese-named methods."""
...
def cantonese_lib_init() -> None:
    ...
    def where(lst: list, index: int, index2=None, index3=None, index4=None):
        if index2 is not None and index3 is None and index4 is None:
            return lst[index][index2]
        if index3 is not None and index2 is not None and index4 is None:
            return lst[index][index2][index3]
        if index4 is not None and index2 is not None and index3 is not None:

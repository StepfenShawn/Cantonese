...
from py_cantonese.can_parser.parser_trait import ParserFn, new_token_context
...
class CanParserContext:
    ...
    def with_name(self, name: str):
        if name == "macro_body":
            from py_cantonese.can_parser import MacroBodyParser

            self.parser = MacroBodyParser
        elif name == "macro_pat":
            from py_cantonese.can_parser import MacroPatParser

            self.parser = MacroPatParser
        elif name == "exp":
            from py_cantonese.can_parser import ExpParser

            self.parser = ExpParser
        elif name == "stat":
            from py_cantonese.can_parser import StatParser
